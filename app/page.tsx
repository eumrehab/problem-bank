...
export default function Home() {
  ...
  function chooseSubject(subject: string) { setSelectedSubject(subject); setView('sets'); window.scrollTo(0, 0); }
  ...
  function reset() { setStudent({ school: '', studentId: '', name: '' }); setSelectedId(''); setSelectedSubject(''); setAnswers({}); setSubmittedAt(''); setView('start'); }
  ...
  return <main className="min-h-screen bg-[#f4f7fb] text-[#182338]">
    ...
    {view === 'subjects' && <section className="mx-auto max-w-3xl px-5 py-9"><button className="back" onClick={() => setView('start')}>← 학생 정보 수정</button><div className="mt-5"><p className="eyebrow">STEP 02</p><h1 className="text-3xl font-black tracking-tight">학습할 파트를 선택하세요</h1><p className="mt-2 text-[#667085]">{student.school} · {student.studentId} · {student.name}</p></div><div className="mt-7 grid gap-4 sm:grid-cols-2"><button onClick={() => chooseSubject('공중보건')} className="subject-card card p-6 text-left"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#e7f3f0] text-3xl">🌿</span><p className="eyebrow mt-5">PART 01</p><h2 className="mt-1 text-2xl font-black">공중보건</h2><p className="mt-2 text-sm leading-6 text-[#667085]">역학, 보건행정, 사회보장과 보건통계를 학습합니다.</p><b className="mt-6 block text-[#176b5b]">공중보건 문제 풀기 →</b></button><button onClick={() => chooseSubject('의료관계법규')} className="subject-card card p-6 text-left"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#eef1ff] text-3xl">⚖️</span><p className="eyebrow mt-5">PART 02</p><h2 className="mt-1 text-2xl font-black">의료법규</h2><p className="mt-2 text-sm leading-6 text-[#667085]">의료법과 의료기사법, 장애인·노인복지 관련 법규를 학습합니다.</p><b className="mt-6 block text-[#176b5b]">의료법규 문제 풀기 →</b></button></div></section>}
